...
describe('ParserService', () => {
    ...
    describe('parseEnvelope', () => {
        ...
        it('should parse alternatives', () => {
            const email: Envelope = {
                header: {
                    messageId: 'messageidAlternative',
                    from: {
                        address: 'test@kalemail.local',
                    },
        ...
        it('should parse alternatives inside parts', () => {
            const email: Envelope = {
                header: {
                    messageId: 'messageidAlternativePart',
                    from: {
                        address: 'test@kalemail.local',
                    },
                    subject: 'Alternative Email',
                ...
                '0': {
                    ...
                    '0': {
                        '0': 'Hello World!',
                        header: {contentType: {mime:'text/plain'}}
                    }
                },
                '1': {
                    header: {
        ...
        it('should parse alternatives attachments', () => {
            const email: Envelope = {
                header: {
                    messageId: 'messageidAlternativeAttachment',
                    from: {
                        address: 'test@kalemail.local',
                    },
                    subject: 'Alternative Email',
                ...
                '0': {
                    ...
                    '0': {
                        '0': {type: 'Buffer', data: new Buffer('Hello World!')},
                        header: {
                            contentType: {mime:'application/octet-stream', name: 'test.attach'},
                            contentDisposition: {mime: 'attachment'}
                        }
                    }
                },
                '1': {
            ...
            expect(parsed.attachments).to.have.length.at.least(1, 'no attachments!');
            expect(parsed.attachments[0]).to.contain.keys(['filename', 'content']);
            expect(parsed.attachments[0].filename).to.equal('test.attach', 'filename is incorrect');
            expect(parsed.attachments[0].content).to.equal(new Buffer('Hello World!').toString('base64'), 'attachment content is incorrect');
        });
